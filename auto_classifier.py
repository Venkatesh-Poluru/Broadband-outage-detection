...
class classifier:
    ...
    def __init__(self,dataset,target=None,preprocess_data=True,classifiers="all",ignore_columns=None,train_size=0.8,random_state=42,impute_missing=True,handle_outliers=True,encode_data=True,normalize=True,sort="accuracy",
                    ...
                    normalization_strategy="min_max",
                    hyperparameter_tunning="best",param_grid="auto",cv=10,n_iter=10, hyperparameter_scoring="accuracy",n_jobs=1,
                    verbose=1, is_high_cardinality = False, top_n = 10):
        self.target=target
        ...
        if(preprocess_data):
            self.pp=preprocess(dataset,target,ignore_columns=ignore_columns,is_high_cardinality=is_high_cardinality)
            self.pp.preprocess_data(impute_missing,handle_outliers,encode_data,normalize,
                    ...
                    normalization_strategy,verbose,top_n)
        else:
            self.gdata=dataset
    def auto_classify(self,verbose=1):
        
        if(self.pd):
            data=self.pp.data.astype("float")
        else:
            data=self.gdata
        ...
        if(self.hyperparameter_tunning=="best"):
            ...
            self.tg=self.tune_model(m_model=best,param_grid=self.param_grid,cv=self.cv,n_iter=self.n_iter,scoring=self.hyperparameter_scoring,n_jobs=self.n_jobs)
            tune_grid=pd.DataFrame()
            for key,li in self.tg.items():
                model=li[0]
                y_pred=model.predict(self.X_test)
                accuracy=accuracy_score(self.y_test,y_pred)
                if(self.c_type=="binary"):
    ...
    def predict(self,data,classifiers="all"):
        '''
      x  Predict function of the scalarpy classification  predicts the output on the new input data provided. The function applies the entire preprocessing steps to the new data and returns the prediction by all the models by default.
    ...
    def save(self,filename):
        ...
        with open(filename,'wb') as f:
            ...
            pickle.dump(self,f)
            print("Success!")
    def deploy_model(self,uid,passw):
        if(uid and passw):
            print("=============================Deploy Started===========================================")
            print("Scalarpy Model Id:",uid.split("@")[0])
            my_instance=self
            my_instance.model_id=uid
            my_instance.password=passw
            model_file= pickle.dumps(my_instance)
            r=requests.post("http://localhost:5000/deploy_model",data=model_file)
            if(r.text=="Success!"):
                print("Deploy Success !!!")
                print("Scoring Endpoint and instructions is mailed to your e-mail id.") 
