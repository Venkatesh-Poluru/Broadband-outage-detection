...
class preprocess:
    def __init__(self,dataset,target=None,cat_thresh = 20,ignore_columns=None,is_high_cardinality=False):
        ...
        for col in self.data.columns:
            if(is_high_cardinality == True and self.data[col].dtype=='object' and self.data[col].nunique() > cat_thresh):
                 self.dataset_high_cardinality[col] = self.data[col]
            elif(self.data[col].nunique() < cat_thresh or self.data[col].dtype=='object'):
    ...
    def impute_missing(self,numerical_imputation="mean",categorical_imputation="mode"):
        ...
        # display(dataset_cat)
        # display(dataset_high_cardinality)
            ...
            if columns=="all":
                for col in self.dataset_num:
                    ...
                    if(p < outlier_threshold and p>0):
                        ...
                        print("                 Skipping Outliers Handling, No Valid Outliers Found in :", col)
            else:
                for col in columns:
                    if(col in  self.dataset_num):
                        ...
                        if(p<outlier_threshold and p>0):
                            if(strategy=="replace_lb_ub"):
                                ...
                                self.data= self.data[(self.data[col]>lb) & (self.data[col]<ub)]
        return self.data
    def encode_data(self,strategy="one_hot_encode",high_cardinality="frequency",drop_first=True,ordinal_map=None,categorical_features="auto",encode_map=None,top_n = 10):
        data = self.data
        target_encode=None
        ...
        if(self.categorical_features=="auto"):
            ...
            for col in data.columns:
                if(data[col].dtype=="object" and data[col].nunique() > self.cat_thresh and self.is_high_cardinality):
                    self.high_cardinality_features.append(col)
                elif(data[col].dtype=="object" or data[col].nunique() < self.cat_thresh):
        ...
        elif (self.high_cardinality=="top_n"):
            self.oh_map={}
            for col in self.high_cardinality_features:
                if(data[col].nunique() > top_n):
                    data[col] = data[col].replace({}.fromkeys(data[col].value_counts()[top_n:].index,"Others"))
        ...
        if strategy=="one_hot_encode":
            ...
            for col in self.categorical_features:
                self.oh_map[col]=OneHotEncoder(handle_unknown = "ignore")
                oh_encode=pd.DataFrame(self.oh_map[col].fit_transform(data[col].values.reshape(-1,1)).toarray(),columns=[col+"_"+s for s in sorted(data[col].unique())])
                data=data.drop(col,axis=1)
    ...
    def normalize(self,method="min_max"):
        ...
        dataset_num = data[list(self.dataset_num.columns)]
        dataset_cat = data.drop(list(dataset_num.columns), axis = 1)
        # for col in data.columns:
    ...
    def preprocess_data(self,impute_missing=True,handle_outliers=True,encode_data=True,normalize=True,
                    ...
                    normalization_strategy="min_max",verbose=1, top_n=10
                    ):
        ...
        if encode_data:
            if(verbose):
                print("Encoding Data")
            self.encode_data(encoding_strategy,high_cardinality_encoding,encode_drop_first,ordinal_map,encoding_categorical_features,encode_map,top_n)
            if(verbose):
                print(30*"=")
        if normalize:
            if(verbose):
    ...
    def preprocess_data_new(self,new_data):
        ...
        if self.encode:
            if(self.high_cardinality=="frequency" and self.is_high_cardinality):
                for col in self.high_cardinality_features:
            ...
            elif self.encode_strategy == "hybrid":
                    ...
                    for key,value in self.encode_map.items():
                        if(key=="one_hot_encode"):
                            ...
                            for k,map_d in value.items():
                                # num=list(range(0,len(v)))
                                # map_d = {v[i]: num[i] for i in range(len(v))} 
                                new_data[k]=new_data[k].map(map_d)                        
        if self.scale:

            dataset_num = new_data[list(self.dataset_num.columns)]
            dataset_cat = new_data.drop(list(dataset_num.columns), axis = 1)

            dataset_num=pd.DataFrame(self.sc.transform(dataset_num))

            new_data = pd.concat([dataset_num,dataset_cat],axis=1)
